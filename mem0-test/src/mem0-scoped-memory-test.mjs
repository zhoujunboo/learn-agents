...
// 添加会话级别记忆，存储特定对话的上下文和目标（userId + runId）
...
// 查询会话级别记忆，使用多条件过滤（user_id + run_id）
...
// 添加 Agent 级别记忆，存储 Agent 的角色定位和行为准则
...
// 查询 Agent 级别记忆，检索 Agent 的角色定位和回答风格
...
// 主程序入口，根据命令行参数执行 add/search/cleanup 操作
async function main() {
