...
import dedent from "dedent"; // 去除模板字符串缩进，便于书写多行提示词
import { ChatOpenAI } from "@langchain/openai";
import { createCodeInterpreterMiddleware } from "@langchain/quickjs"; // 提供 eval REPL 代码执行能力
import { createDeepAgent, FilesystemBackend } from "deepagents";

import { webSearch } from "./tools/search.mjs"; // 联网搜索工具

// 项目根目录：从当前文件（src/）向上一级定位，用作文件系统后端与技能/记忆的根路径
const projectDir = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  "..",
);

// 调研员子 Agent：唯一持有联网搜索工具，负责调研单一子主题并输出一份 findings 文件
const researcherSubAgent = {
  ...
  `,
  tools: [webSearch], // 仅调研员可联网搜索
};

// 编辑子 Agent：只审阅草稿并给出修改意见，不直接改写报告（无额外工具）
...
// 分析师子 Agent：通过 eval REPL 做数值计算与数据分析，禁止凭空猜测数字
const analystSubAgent = {
  ...
  `,
  middleware: [createCodeInterpreterMiddleware()], // 注入代码执行中间件，提供 eval REPL
};

// 主 Agent（编排器）系统提示：定义整体流程、委派规则与文件约定
...
export function createIntelligenceDeskAgent() {
  ...
  // 模型与自定义接口地址可通过环境变量覆盖，默认使用 gpt-4o
  const model = process.env.MODEL_NAME?.trim() || "gpt-4o";
  const baseURL = process.env.OPENAI_BASE_URL?.trim() || undefined;

  // 文件系统后端：virtualMode 下所有读写限定在虚拟工作区，不直接落盘到真实项目目录
  const backend = new FilesystemBackend({
    rootDir: projectDir,
    virtualMode: true,
  });

  // 聊天模型：temperature=0 保证输出稳定；仅在提供 baseURL 时注入自定义接口配置
  ...
  // 第二次执行 上下文大小限制 到了这里85% 会压缩 8K
  // 会触发摘要 后把，会话原文记录在 conversation_history 目录下归档
  Object.defineProperty(chatModel, "profile", {
    get: () => ({ maxInputTokens: 8_000 }),
  });

  // 创建深度 Agent，装配模型、提示词、文件后端、长期记忆、技能目录与三个子 Agent
  return createDeepAgent({
    model: chatModel,
    systemPrompt: orchestratorPrompt,
    backend,
    memory: [path.join(projectDir, "AGENTS.md")], // 加载长期记忆文件
    skills: ["/skills/"], // 技能目录（web-research、report-writer 等写作指南）
